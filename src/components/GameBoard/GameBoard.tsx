  ...
  for (let y = bounds.minY; y <= bounds.maxY; y++) {
    for (let x = bounds.minX; x <= bounds.maxX; x++) {
      ...
      cells.push(
        <div
          key={`${x},${y}`}
          className={`${styles.cell} ${hasCard ? styles.occupied : ''} ${
            isHovered ? styles.hovered : ''
          } ${isStarter ? styles.starter : ''} ${isPreview ? styles.preview : ''} ${
            isNotAllowed ? styles.notAllowed : ''
          } ${isInvalidPlacement ? styles.invalidPlacement : ''}`}
